            ...
            for row in rooms:
                ...
                try:
                    ...
                    if "404" in str(e):
                        ...
                        # 在这里你可以选择将这个无效房间从本地数据库删除
                        # cursor.execute("DELETE FROM rooms WHERE room_id = ?", (room_id,))
                    else:
                        # 如果是其他错误 (如 401, 500)，则打印并停止
                        print(f"处理房间 '{room_id}' 时发生严重错误，请检查: {e}")
                        # 可以在这里决定是否要回滚所有操作
                        # db.rollback()
                        # return # 提前退出函数
            
            db.commit() # 在所有操作完成后提交一次
            print("房间同步任务完成！")
...
async def upload_file(image: Image.Image, prompt: str, room_id: str, image_key: str):
    ...
    # 构建S3对象的直接访问URL
    s3_url = f"https://{AWS_S3_BUCKET_NAME}.s3.{region}.amazonaws.com/{key_name}"
    out = {"url": s3_url,
           "filename": filename}
    # --- END OF CHANGE ---
    return out
...
async def create_upload_file(file: UploadFile):
    ...
    # --- START OF CHANGE ---
    # 定义对象键以便复用
    object_key = "community/" + file.filename
    s3.upload_fileobj(Fileobj=temp_file, Bucket=AWS_S3_BUCKET_NAME, Key=object_key, ExtraArgs={"ContentType": file.content_type, "CacheControl": "max-age=31536000"})
    temp_file.close()

    # 获取S3客户端的区域以构建正确的URL
    region = s3.meta.region_name
    # 构建S3对象的直接访问URL
    s3_url = f"https://{AWS_S3_BUCKET_NAME}.s3.{region}.amazonaws.com/{object_key}"
