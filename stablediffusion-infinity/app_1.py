...
async def run_outpaint(
    ...
    try:
        ...
        # 新增：检测选区中非空白区域的比例
        if mask.sum() > 0:  # 只有当蒙版存在时才检查
            # 计算蒙版中非零像素(非透明)的比例
            mask_ratio = np.count_nonzero(mask) / mask.size
            
            # 如果选区中大部分是空白(非透明像素比例低)
            if mask_ratio < 0.2:  # 如果选区中少于20%是非透明的，认为是空白区域
                logger.info(f"[Request ID: {request_id}] 检测到选区大部分为空白，将执行纯文本生成而非inpaint。")
                # 在选区中创建随机噪声作为初始图像
                # 保留选区位置，但填充随机噪声
                random_img = np.random.randint(0, 256, img.shape, dtype=np.uint8)
                # 创建全白蒙版，表示整个选区都需要重新生成
                mask_full = np.zeros_like(mask)
                mask_full[mask > 0] = 255  # 只在原蒙版区域生成
                mask = mask_full
                # 将原始图像与随机噪声结合
                nmask = mask.copy()
                nmask[mask > 0] = 1
                img = nmask[:, :, np.newaxis] * random_img + (1 - nmask[:, :, np.newaxis]) * img
                # 设置更高的强度值
                strength = 1.0
                logger.info(f"[Request ID: {request_id}] 已设置强度为1.0以进行完全重新生成。")
